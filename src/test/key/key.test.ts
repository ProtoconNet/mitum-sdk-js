...
test("test: key weighted public key", () => {
    const tcs = [
        {
            weight: 1,
    ...
    tcs.forEach((tc) => {
        const key = new PubKey(tc.key, tc.weight)
...
test("test: key address from keys", () => {
    const tcs = [
        {
            keys: [
                {
    ...
    tcs.forEach((tc) => {
        const ks = tc.keys.map((k) => new PubKey(k.key, k.weight))
...
test("test: key duplicate keys", () => {
    const pub = "kpZLdVYFJXVbS7aXkVi4qeYutZu5fFYH9Zber1TYEkuempu"

    expect(
        () =>
            new Keys([new PubKey(pub, 50), new PubKey(pub, 50)], 100)
    ).toThrow(Error)
})