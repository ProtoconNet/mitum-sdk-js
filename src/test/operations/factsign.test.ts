...
test("test: factsign m2", () => {
	...
	tcs.forEach((tc) => {
		...
		const kp = M2KeyPair.fromPrivateKey(tc.key)
		...
		time.toBuffer().forEach((b, idx) => expect(b).toBe(tc.nbytes[idx]))
		expect(base58.encode(sign)).toBe(tc.sign)
	})
})
